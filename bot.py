            ...
            try:
                async with self.session.get(req, headers=headers) as response:
                    if response.status == 404:
                        ...
                        logging.error(
                            f"Error fetching WiGLE user stats for {username}: {response.status}"
                        )
                        return {
                            "success": False,
                    ...
                    if (
                        ...
                        and "userName" in data["statistics"]
                    ):
                        if data["statistics"]["userName"].lower() == username.lower():
                            ...
                            if image_url:
                                ...
                                data[
                                    "imageBadgeUrl"
                                ] = image_url  # Update the URL in the data dictionary
                        ...
                        return {
                            "success": False,
                            "message": "Invalid data received or user not found.",
                        }
            except Exception as e:
        ...
        try:
            async with self.session.get(req, headers=headers) as response:
                if response.status != 200:
                    logging.error(
                        f"Error fetching WiGLE group ranks: {response.status}"
                    )
                    return {
                        "success": False,
                        "message": f"HTTP error {response.status}",
    ...
    async def fetch_wigle_id(self, group_name: str):
        ...
        try:
            async with self.session.get(req, headers=headers) as response:
                if response.status != 200:
                    logging.error(
                        f"Error fetching WiGLE group ID for '{group_name}': {response.status}"
                    )
                    return {
                        "success": False,
                        "message": f"HTTP error {response.status}",
                ...
                if "success" in data:
                    ...
                    logging.warning(
                        f"WiGLE group ID fetch error for '{group_name}': {data['message']}"
                    )
    ...
    async def fetch_user_rank(self, url: str):
        try:
            async with self.session.get(url) as response:
                if response.status != 200:
                    logging.error(
                        f"Error fetching user rank from URL: {url}, HTTP error {response.status}"
                    )
                    return None
...
async def user(interaction: discord.Interaction, username: str):
    ...
    try:
        ...
        if "success" in response and response["success"]:
            ...
            image_url = response["imageBadgeUrl"]

            # Create an embed object for a nicely formatted Discord message
            embed = discord.Embed(
                title=f"WiGLE User Stats for '{username}'", color=0x1E90FF
            )
            ...
            embed.add_field(
                name="Events This Month", value=eventMonthCount, inline=True
            )
            embed.add_field(
                name="Last Month's Events", value=eventPrevMonthCount, inline=True
            )
            embed.add_field(
                name="Discovered WiFi GPS", value=discoveredWiFiGPS, inline=True
            )
            embed.add_field(
                name="Discovered WiFi GPS Percent",
                value=discoveredWiFiGPSPercent,
                inline=True,
            )
            embed.add_field(name="Discovered WiFi", value=discoveredWiFi, inline=True)
            embed.add_field(
                name="Discovered Cell GPS", value=discoveredCellGPS, inline=True
            )
            embed.add_field(name="Discovered Cell", value=discoveredCell, inline=True)
            embed.add_field(
                name="Discovered BT GPS", value=discoveredBtGPS, inline=True
            )
            embed.add_field(name="Discovered BT", value=discoveredBt, inline=True)
            embed.add_field(
                name="Total WiFi Locations", value=totalWiFiLocations, inline=True
            )
            ...
            logging.warning(
                f"WiGLE user stats fetch error for {username}: {error_message}"
            )
        ...
        await interaction.followup.send(
            f"Error: A required piece of information is missing: {e}"
        )
...
async def grouprank(interaction: discord.Interaction):
    ...
    if "success" in response and response["success"] is True:
        ...
        for i, group in enumerate(groups[:40], 1):  # zzTop 40
            ...
            embed = discord.Embed(
                title="WiGLE Group Rankings",
                description=rankings,
                color=EMBED_COLOR_GROUP_RANK,
            )
        await interaction.followup.send(embed=embed)
    else:
        await interaction.followup.send(
        ...
        if "success" in response and response["success"] is True:
            ...
            if url is not None:
                ...
                if group_data:
                    ...
                    await interaction.followup.send(
                        "Failed to fetch group data from the URL."
                    )
            else:
                logging.warning(
                    f"Missing 'url' key in WiGLE API response for '{group}'"
                )
                await interaction.followup.send(
                    "Invalid API response: missing 'url' key."
                )
        else:
...
@client.tree.command(
    name="help", description="Displays help information for WiGLE Bot commands."
)
